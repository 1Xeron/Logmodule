const chalk = require('chalk'), fs = require('fs'), DATE = { FORMAT_DATE: "dd/mm/yyyy HH:MM:ss", datePrefix: "[", dateSuffix: "]", labelPrefix: "[", labelSuffix: "]" };
require(__dirname + '/index.js');

var util = require('util'), os = require('os'), dateFormat = require('dateformat'), now = new Date(), log_file_stdout = fs.createWriteStream(__dirname + '/log/stdout.log', { flags: 'w' }), log_file_stderr = fs.createWriteStream(__dirname + '/stderr.log', { flags: 'w' }),
    log_stdout = process.stdout, log_stderr = process.stderr, mem = Math.round(process.memoryUsage().rss / 1024 / 1024), cpu = os.loadavg(),
    ...
    log = {
        ...
        warn: function (warn) {
            log_stdout.write(chalk.cyan(dateFormat(now, `${DATE.datePrefix}` + DATE.FORMAT_DATE)
                + `${DATE.dateSuffix} ${DATE.labelPrefix} CPU: ${Math.ceil(cpu[1] * 100) / 10 + " %"} Mem: ${mem} MB ${DATE.labelPrefix}WARN${DATE.labelSuffix}`) + ` ` + util.format(chalk.yellow(warn + `\n`)));
